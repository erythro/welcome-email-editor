	...
	foreach ($page_options as $name=>$options) {
		...
		switch ($options['type']) {
			...
			case 'textarea':
				...
				break;
			case 'wysiwyg':
				$html .= sb_we_get_wysiwyg($name, $value, $options['class'], $options['style'], $options['rows'], $options['cols']);
				break;
			case 'select':
...
function sb_we_get_wysiwyg($name, $value, $class=false, $style=false, $rows=false, $cols=false) {
	#$style = ($style ? ' style="' . $style . '"':'');
	#$class = ($class ? ' class="' . $class . '"':'');
...
// Prevent plugin from checking for updates
